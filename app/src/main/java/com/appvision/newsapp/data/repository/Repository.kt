...
class Repository(private val webService: WebService, private val dao: DAO) {
    ...
    suspend fun getTopHeadlines() = webService.getTopHeadlines("us", Constants.API)

    suspend fun getAllArticles(title: String) = webService.getAllArticles(title, Constants.API)
    ...
    fun loadBookmarkArticle(id: String): LiveData<List<ArticleModel>> = dao.loadBookmarkArticle(id)
